...
export default function ProductCategoryEdit() {
    ...
    const { data, setData, put, processing } = useForm({
        name: category.name ?? '',
        slug: category.slug ?? '',
        catalog_id: String(category.catalog_id ?? ''),
        parent_id: category.parent_id ? String(category.parent_id) : '',
        position: category.position ?? 0,
    });

    const filteredCategories = categories.filter((item) => !data.catalog_id || item.catalog_id === Number(data.catalog_id));
    ...
    return (
        <>
            ...
            >
                <div>
                    <h1 className="text-lg font-semibold text-gray-900">Edycja kategorii</h1>
                    <p className="text-sm text-gray-500">Zaktualizuj dane kategorii produktów.</p>
                </div>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <Button type="button" variant="ghost" asChild>
                        <Link href="/product-categories">Wróć</Link>
                    </Button>
                </div>
            </form>
        </>
